...
public class HandlerRunnable implements Runnable {
    ...
    public HandlerRunnable(Socket clientSocket) {
    ...
    public void run() {
        ...
        if (request.getIsWS()) {
            makeHandshake(clientSocket_, request);

            while (true) {
                try {
                    InputStream is  = clientSocket_.getInputStream();
                    DataInputStream dis = new DataInputStream(is);
                    String wsRequest = readWebSocketMessage(dis);
                    ...
                    handleResponse(clientSocket_, wsRequest);
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            Response response = new Response(clientSocket_, request);
    ...
    private static String readWebSocketMessage(DataInputStream dataInputStream) throws IOException {
        ...
        boolean isMasked = (incomingBytes.get(1) & 0x80) != 0;
        boolean isClosingFrame = (incomingBytes.get(0) & 0x0f) == 8;
        ...
        for (int i = 0; i < payloadLength; i++) {
            payload[i] = incomingBytes.get(i + payloadStart);
        }
        ...
        if (isMasked) {
            ...
            for (int i = 0; i < 4; i++) {
                masks[i] = incomingBytes.get(i + payloadStart - 4);
            }
    ...
    private static void broadcastResponse(Map<String, String> responseJSON, Socket socket) throws IOException {
        ...
        if (Objects.equals(responseJSON.get("type"), "join")) {
            ...
            for (int i = 0; i < messageQueue.size() - 1; i++) {
                sendResponse(stringifyJSON(messageQueue.get(i)), socket);
            }
        }
    ...
    private static void handleResponse(Socket socket, String request) throws IOException {
    ...
    private static String getWebSocketResponseKey(String requestKey) throws NoSuchAlgorithmException {
    ...
    private static byte[] getResponseFrame(String response) {
    ...
    private static void makeHandshake(Socket socket, Request request) {
        OutputStream os;
        try {
        ...
        try {
            response = ("HTTP/1.1 101 Switching Protocols\r\n"
                    ...
                    + getWebSocketResponseKey(request.getKey())
